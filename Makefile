...
test-coverage:  ## run tests and generate coverage report
	py.test --cov-report term-missing --cov=yagdal -v -s
...
install-dev: clean ## install development version to active Python's site-packages
	...
	YAGDAL_FULL_COVERAGE=YES python -m pip install -e . --no-use-pep517 || YAGDAL_FULL_COVERAGE=YES python -m pip install -e .
